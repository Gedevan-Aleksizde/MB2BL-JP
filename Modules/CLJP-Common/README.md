...
最新バージョン: 0.9.0, 本体 ver. 1.0.3 に対応
...
2. 日本語のテキストも変な箇所が多いので修正します. 本体のテキスト全体の約2万件のうち, 現在は **21%くらい**を修正しています.
    * このパートは, 別のゲームの話になってしまいますが, [スカイリムの日本語校正mod](https://tktk1.net/skyrim/mymod/improved-japanese-translation/) のコンセプトに近いです.
...
1. Optional Files の CorrectLocalizationJP-Overwriter-* のいずれか1つをダウンロードし フォルダ内の複数のフォルダを上書きしてください.
2. ランチャーで CorrectFontLocalizerJP-* を有効にしてから起動してください. 言語設定は「日本語」のままにしてください.
...
* システムメッセージやアイテム名に見られる悪文 (とても多いです)
    * 例: 『毛皮の鎧（ストラップ付き）』
    * 例: 『鱗下地の装飾された革の引き具』 (こっちは誤訳も含んでいます)
* 文体の統一
    * ですます調とである調が交互に現れたり, 説明文で急にくだけた口調になったりする文を修正
...
1. メニュー画面, マップや戦闘時のステータス表示・ログ等のUI関連 - 95% ~~90%, ほぼ完了~~
      * パークスキルの「~~~~がXX%増加(減少)」のような文言の表現を簡潔かつわかりやすくした上で, 定型文に統一するなどしています. 100%にならないのは本体の制約上, テキストの書き換えだけでは難しい箇所があるため
2. オプションの設定項目関連 - 50%くらい?
3. アイテム名・鍛冶の部品名 - 95% ~~80%, おおむねできています, 一般的な表記ルールを整備するかもしれない~~
      * 必要に応じて実物を確認した上でわかりやすい表現に直し, ある程度規則性のある命名に整備しました.
4. クエスト・会話セリフ・百科事典のテキスト - 10% 正しくない用語や誤字を機械的に置換した程度でほとんど手つかず
5. 固有名詞・汎用人名のスペル確認 - 1% 機械的な置換のみ
6. マルチプレイ - ??%, Module版では動作しない・そもそもあまりやってないため確認中
...
* 戦闘関連は特に変なやつが多いです
    ...
    * Four Part/四部構成 -> 十字型
    * Thrre Part/三部構成 -> 三叉型
    * disorganized state recovery/陣形の再結成, その他色々 -> 部隊再編 (戦闘終了直後にマップ上で移動速度その他のペナルティが発生している状態)
    ...
    * Chamber Blocked!/部屋が封鎖されている! -> チャンバーブロック! (閉じ込められた時ではなく, チャンバーブロックした/されたときのログメッセージです)
...
* Q.: マジで文字化けしなくなるの?
* A.: 通常の日本語であれば多分大丈夫です. 例えば仮に, modの翻訳文に「『颯』爽」「樽」「隼」「憂『鬱』」「『驃』騎兵」といった常用漢字外の字を追加したとしても表示されます. 厳密なテストはしていませんが, 源真ゴシックは 19,623, 源瑛ラテミンは 18,198 のグリフを収録しています. いずれもAJ1-6 に準拠した源ノ角ゴシックを元にしているため, 通常日本語で使われうる文字はほぼ全て対応していると考えて問題ないです. 逆に, 簡体字などは対応してないですが, AJ1-6のグリフ数でもけっこうギリギリだったので現状M&B2 の仕様に沿ってCJK統合漢字を全て同じスタイルで表示させるのは不可能だと思います.
