...
import pandas as pd
import polib
from functions import initializePOFile, merge_yml, update_with_older_po
...
parser.add_argument(
    ...
    nargs="*",
    default=None,
    help="module names what you want to read.",
)
parser.add_argument(
    "--output",
    type=Path,
...
parser.add_argument(
    ...
    type=Path,
    default=None,
    help='older .PO file path. the original text will be merged and updated with this if exists. Default: what you specified in--output with suffix "-old.po"',
)
parser.add_argument(
    "--only-diff",
    type=bool,
...
parser.add_argument("--locale", default=None, type=str)
parser.add_argument("--distinct", default=None, action="store_true")
...
def main(args: argparse.Namespace):
    ...
    if args.legacy_id:
        ...
        df_new = df_new.assign(
            id_original=lambda d: d["id"], id=lambda d: d["id"] + "/" + d["text_EN"]
        )
    df_new[f"text_{args.langshort}_original"] = df_new[
        f"text_{args.langshort}_original"
    ].fillna("")
    ...
    if args.distinct:
        ...
        duplicates = (
            df_new[["id", "file", "module"]]
            .assign(locations=lambda d: (d["module"] + "/" + d["file"]).astype(str))
            .groupby("id")
            .agg(
                {
                    "locations": [
                        lambda locs: [(x, 0) for x in locs],
        ...
        if args.duplication_in_comment:
            df_new = df_new.assign(
                notes=lambda d: np.where(
                    d["duplication"] > 1,
                    [
                        ",".join(
                            [
                                x
                                for x in [note, f"""{ndup} ID duplications"""]
                                if x != ""
                            ]
                        )
                        for note, ndup in zip(d["notes"], d["duplication"])
                    ],
                    d["notes"],
                )
            )
    ...
    if args.only_diff:
        for _, r in df_new.iterrows():
            new_pof.append(polib.POEntry(msgid=r["id"], flags=["fuzzy"]))
    else:
        for _, r in df_new.iterrows():
            new_pof.append(
                polib.POEntry(
    ...
    if args.output.exists():
        backup_path = args.output.parent.joinpath(
            f"""{args.output.with_suffix('').name}-{datetime.now().strftime("%Y-%m-%dT%H%M%S")}.po"""
        )
        args.output.rename(backup_path)
        print(f"""old file is renamed to {backup_path.name}""")
    new_one.save(args.output)
...
def read_xmls(args: argparse.Namespace, how_join="left") -> pd.DataFrame:
    ...
    for module in args.vanilla_modules:
        dp = (
            args.mb2dir.joinpath("Modules")
            .joinpath(module)
            .joinpath("ModuleData/Languages")
        )
                    ...
                    d[args.langshort] += [
                        pd.DataFrame(
                            [
                                ...
                                for x in xml.findall("strings/string")
                            ],
    ...
    d[args.langshort][f"text_{args.langshort}_original"] = d[args.langshort][
        f"text_{args.langshort}_original"
    ].str.replace("[\u00a0\u180e\u2007\u200b\u200f\u202f\u2060\ufeff]", "", regex=True)
    # d[args.langshort]['file'] = d[args.langshort]['file'].str.replace(r'^(.+)_jpn\.xml', r'\1.xml', regex=True)
    d[args.langshort] = d[args.langshort].rename(
        columns={"file": f"file_{args.langshort}", "module": f"module_{args.langshort}"}
    )
    d_bilingual = d["EN"].merge(d[args.langshort], on=["id"], how=how_join)
    d_bilingual = d_bilingual.assign(
        file=lambda d: np.where(
            d["file"].isna(), "Hardcoded, " + d[f"file_{args.langshort}"], d["file"]
        ),
        module=lambda d: np.where(
            d["module"].isna(),
            "Hardcoded, " + d[f"module_{args.langshort}"],
            d["module"],
        ),
    )
    ...
    if how_join:
        d_bilingual = d_bilingual.assign(
            notes=lambda d: np.where(
                d["text_EN"].isna(),
                "The original text cannot available, assumed hardcoded",
                "",
            ),
            text_EN=lambda d: np.where(
                d["text_EN"].isna(), d[f"text_{args.langshort}_original"], d["text_EN"]
            ),
        )
    else:
        d_bilingual["notes"] = ""
    d_bilingual = d_bilingual[
        ["module", "file", "id", "text_EN", "notes"]
        + [x for x in d_bilingual.columns if (x[:5] == "text_" and x[-2:] != "EN")]
    ].sort_values(["id", "file"])
...
def check_duplication(df_bilingual: pd.DataFrame) -> pd.DataFrame:
    ...
    # TODO: 仕様が古い?
    duplicated_id = (
        df_bilingual[["id", "text_EN"]]
        .groupby(["id"])
        .agg({"text_EN": [pd.Series.nunique, "count"]})
        .reset_index()
    )
    duplicated_id.columns = ["id", "unique", "duplicates"]
    duplicated_id = duplicated_id.loc[
        lambda d: (d["unique"] > 1) | (d["duplicates"] > 1)
    ]
    ...
    df_distinct = df_bilingual.merge(
        duplicated_id[["id", "duplicates", "unique"]].drop_duplicates(),
        on="id",
        how="left",
    ).assign(
...
def drop_duplicates(
    ...
    if file_order is None:
        file_order = ["std_global_strings", "std_module_string"]
    file_order = {k: v for k, v in zip(range(len(file_order)), file_order)}
    default_file_order = len(file_order) + 1
    if compare_module:
        df = df.assign(
            module_order=lambda d: [
                module_order.get(x, default_module_order) for x in d[col_module]
            ]
        )
    if compare_file:
        df = df.assign(
            file_order=lambda d: [
                file_order.get(x, default_file_order) for x in d[col_file]
            ]
        )
    df = (
        df.sort_values(
            ["id"] + (["module_order"] if compare_module else []) + ["file_order"]
            if compare_file
            else []
        )
        .groupby(["id"])
        ...
        .drop(
            columns=(
                (["module_order"] if compare_module else []) + ["file_order"]
                if compare_file
                else []
            )
        )
...
if __name__ == "__main__":
    ...
    if arguments.pofile is None:
        arguments.pofile = Path(arguments.output.parent).joinpath(
            f"MB2BL-{arguments.langshort}.po"
        )
    print(arguments)
